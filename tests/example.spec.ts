...
test('incorpora las correcciones historicas principales', async ({ page }) => {
  ...
  await expect(page.getByText('Cementerio de la Chacarita').first()).toBeVisible();
  await expect(page.getByText('Chacarita de los Colegiales').first()).toBeVisible();
  await expect(page.getByText('Corrales de Miserere').first()).toBeVisible();
  await expect(page.getByText('Cabildo').first()).toBeVisible();
  await expect(page.getByText(/Beresford terminó capitulando/i).first()).toBeVisible();
  ...
  await expect(page.getByText('Parque Los Andes')).toHaveCount(0);
  await expect(page.getByText('Plaza Mayor y el Fuerte')).toHaveCount(0);
  await expect(page.getByText(/se rindió en la Plaza Mayor/i)).toHaveCount(0);
  await expect(page.getByText(/6 paradas/i)).toHaveCount(0);
  await expect(page.getByText('Audio continuidad 06')).toHaveCount(0);
