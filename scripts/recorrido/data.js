const POINTS_URL = "/public/data/points.geojson";
const ROUTE_URL = "/public/data/route.geojson";
const PODCAST_URL = "/public/data/podcast.json";
...
function getIllustrationFallback(order) {
  return `/public/images/points/punto${order}.svg`;
}
...
function normalizePoint(feature, podcastIndex) {
  ...
  const fallbackImage = getIllustrationFallback(order);
  const audioSource = podcast?.audio || properties.audio || "";
  ...
  return {
    ...
    image: properties.image || fallbackImage,
    fallbackImage,
    coordinates,
